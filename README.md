...
`tools/list` at `https://gateway.pipeworx.io/ukri-gtr/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/gtr_search_projects \
  -H 'Content-Type: application/json' \
  -d '{"query":"quantum computing","funder":"EPSRC","status":"Active","limit":10}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/gtr_search_projects`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
